...
# taken from https://github.com/kpmooney/numerical_methods_youtube/blob/master/root_finding/implied_volatility/find_vol_put.py # noqa
...
def compute_delta(
    current_price, strike, interest_rate, days_to_expiry, option_price, is_call
):
    ...
    for _ in range(MAX_ITERATIONS):
        ...
        if is_call:
            value = (
                call_price(volatility, current_price, strike, interest_rate, t, d1, d2)
                - option_price
            )
        else:
            value = (
                put_price(volatility, current_price, strike, interest_rate, t, d1, d2)
                - option_price
            )
